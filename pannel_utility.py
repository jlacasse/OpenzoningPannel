...
#if one of the thermostat fan is on, the fan status is on
def define_system_fan_status():
    all_th_json = json.loads(read_all_mcp_out_json()) 
    for th in all_th_json["zones"]:
        if all_th_json["zones"][th]["g"] == "true":
            return True
        else:
            return False
        
def define_th_mode(th):
    all_th_json = json.loads(read_all_mcp_out_json()) 
    if all_th_json["zones"][th]["y1"] == "false":
        return "off"
    elif all_th_json["zones"][th]["ob"] == "true":
        return "heat"
    else:
        return "cool"